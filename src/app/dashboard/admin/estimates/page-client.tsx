...
type AdminEstimate = Awaited<ReturnType<typeof getAdminEstimates>>[0];

export function AdminEstimatesPageClient({ estimates }: { estimates: AdminEstimate[] }) {
  const [editingEstimate, setEditingEstimate] = useState<AdminEstimate | null>(null);
  const [showEditModal, setShowEditModal] = useState(false);

  const openEditModal = (estimate: AdminEstimate) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-lg max-h-[90vh] overflow-y-auto">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-950">Edit estimate</h2>
              <button
            ...
            <form
              ...
              onSubmit={(e) => {
                ...
                updateEstimateAction(formData).then(() => closeEditModal());
              }}
            ...
            >
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  Estimate ID
                  <input name="estimateId" required defaultValue={editingEstimate.id} disabled className="rounded-2xl border border-slate-300 px-4 py-3 bg-slate-100" />
                </label>
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  Title
                ...
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  Status
                  <select name="status" defaultValue={editingEstimate.status} className="rounded-2xl border border-slate-300 px-4 py-3">
                ...
                <label className="flex items-center gap-2 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-700">
                  <input name="visibleToClient" type="checkbox" defaultChecked={editingEstimate.visibleToClient} className="h-4 w-4 rounded border-slate-300" />
                  Visible to client
                </label>
              </div>
              ...
              <div className="mt-6 flex gap-3">
                <button type="submit" className="flex-1 rounded-full bg-slate-950 px-5 py-3 font-semibold text-white">Update estimate</button>
                <button
                  type="button"
                  onClick={closeEditModal}
              ...
              <form action={deleteEstimateAction} className="mt-2 rounded-2xl border border-red-200 bg-red-50 p-4">
                <input type="hidden" name="redirectTo" value="/dashboard/admin/estimates" />
                ...
                <button className="w-full rounded-full border border-red-300 px-5 py-3 font-semibold text-red-700 hover:bg-red-100">Delete estimate</button>
              </form>
            </form>
          </div>
        </div>
          ...
          {estimates.length > 0 ? estimates.map((estimate) => (
            <div key={estimate.id} className="rounded-2xl bg-slate-50 p-4">
              ...
              <div className="mt-3 flex gap-2">
                <button onClick={() => openEditModal(estimate)} className="rounded-full bg-slate-950 px-4 py-2 text-xs font-semibold text-white">Edit</button>
